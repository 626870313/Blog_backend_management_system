import { Upload, Spin, Button, Input, Form, Modal,Message } from '@arco-design/web-react';
...
const Item = (props) => {
    const { onChange, onRemove, onAdd, index = 0, showImg, showLink, showIcon, showAction, showAdd = true, showReduce = false,
        imgUrl, link, icon } = props

    const [imageUrl, setImageUrl] = React.useState<string>(imgUrl || '');
    ...
    const beforeUpload = async (file) => {
        ...
        if(!isImage){
            return Message.warning('请上传jps,jpeg,png,gif图片')
        }
        ...
        <div className={styles['item-content']}>
...

            {
                showAction && <div className={styles.action}>
                    {
                        showReduce && <Button icon={<IconDelete />} status='danger' shape='circle' className={styles['btn']} onClick={() => onRemove(true)} ></Button>
                    }
                    {
                        showAdd && <Button icon={<IconPlus />} shape='circle' className={styles['btn']} type='primary' onClick={onAdd}></Button>
                    }
                </div>
            }
